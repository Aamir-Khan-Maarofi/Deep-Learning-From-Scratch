...
class Dense():
    ...
    def __init__(self, n_inputs, n_nodes):
        ...
        self.weights = np.random.uniform(
            low=-0.1, high=0.1, size=n_inputs*n_nodes).reshape(n_inputs, n_nodes)
        ...
        self.biases = np.random.uniform(low=0.1, high=0.2, size=n_nodes)
    ...
    def forward_pass(self, inputs):
        ...
        self.activation_potentials = np.dot(
            self.weights.T, inputs) + self.biases
    ...
    def backward_pass(self, learning_rate, inputs_to_layer, target,
        ...
        if not len(prev_loc_grads):
            ...
            self.error_at_end_nodes = target - self.outputs
        ...
        self.weights = self.weights + \
            (learning_rate * np.outer(inputs_to_layer, self.loc_gradients))
